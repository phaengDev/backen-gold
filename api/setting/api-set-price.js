...
router.post("/create", function (req, res) {
    ...
    upload(req, res, function (err) {
        ...
        const priceSale = parseFloat(price_sale.replace(/,/g, '')); // e.g., '1977333.33'
        const decrease_sale = parseFloat(Decrease.replace(/,/g, '')); // also limited to 2 decimal places
...
router.post("/", function (req, res) {
    ...
    const wheres = `prices_id !=''  ${type_Id_fk}  ORDER BY type_id_fk ASC`;
...
router.get("/sale/:id", function (req, res) {
    const typeId_fk = req.params.id;
    const wheres = `tbl_price_gold.type_id_fk = '${typeId_fk}'`;
    const fields = `tbl_price_gold.price_buy,tbl_price_gold.price_sale,tbl_price_gold.type_id_fk,tbl_type_gold.typeName,
    (SELECT grams FROM tbl_options WHERE option_id='1') AS grams,
    ((SELECT grams FROM tbl_options WHERE option_id='1') * tbl_price_gold.price_sale) AS price_sale `;
    const tables = `tbl_price_gold
LEFT JOIN tbl_type_gold ON tbl_price_gold.type_id_fk = tbl_type_gold.type_Id`;
    db.selectWhere(tables, fields, wheres, (err, results) => {
        if (err) {
...
router.get("/prices", function (req, res) {
    const tables=`tbl_price_gold
	INNER JOIN tbl_type_gold ON tbl_price_gold.type_id_fk = tbl_type_gold.type_Id`;
    const fields=`*,
	(SELECT grams FROM tbl_options WHERE tbl_options.option_id=1) AS grams`;
    const wheres=`type_Id=2`;
